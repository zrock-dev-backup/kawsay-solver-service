...
from src.solver_service.handlers.preference_handler import PreferenceConstraintHandler
...
def test_time_slot_preference_adds_penalty(solve_model):
    ...
    act = problem.activities.add(
        id="A1", duration_in_slots=1, time_preference_id="pref1", is_locked=True
    )
    act.locked_start_time.day_index = 0
    act.locked_start_time.slot_index = 1

    modeler = Modeler(problem)
    handler = PreferenceConstraintHandler(modeler)
    handler._apply_time_slot_preferences(problem)
...
def test_student_gap_penalty_is_applied(solve_model):
    """Verifies goal is penalized for gaps in a student's schedule."""
