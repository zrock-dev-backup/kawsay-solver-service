...
def test_day_boundary_is_enforced(solve_model):
    ...
    structural._apply_day_boundary_constraints(modeler, problem)
...
def test_activity_ordering_is_enforced(solve_model):
    ...
    problem.activities.add(id="A2", duration_in_slots=1, is_locked=True, locked_start_time={'day_index': 0, 'slot_index': 0})

    modeler = Modeler(problem)
    structural._apply_activity_relation_constraints(modeler, problem)
    result = solve_model(modeler)
...
def test_consecutive_activities_are_enforced(solve_model):
    ...
    problem.activities.add(id="A1", duration_in_slots=1, is_locked=True, locked_start_time={'day_index': 0, 'slot_index': 0})
    problem.activities.add(id="A2", duration_in_slots=1, is_locked=True, locked_start_time={'day_index': 0, 'slot_index': 2})
...
def test_min_days_between_is_enforced(solve_model):
    ...
    problem = conftest.create_problem(days=2, slots_per_day=1)
    problem.min_days_between_activities.add(first_activity_id="A1", second_activity_id="A2", minimum_days=2)
    # Lock activities on adjacent days, violating the min_days=2 rule
    problem.activities.add(id="A1", duration_in_slots=1, is_locked=True, locked_start_time={'day_index': 0, 'slot_index': 0})
    problem.activities.add(id="A2", duration_in_slots=1, is_locked=True, locked_start_time={'day_index': 1, 'slot_index': 0})
